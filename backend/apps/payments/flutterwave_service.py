...
class FlutterwaveService:
    ...
    def __init__(self, base_url: str, secret_key: str):
        ...
        Args:
            ...
            secret_key: Flutterwave secret key for authentication
    ...
    async def initiate_payment(
        ...
        logger.info(
            f"Initializing Flutterwave payment for transaction {transaction_reference}"
        )
    ...
    async def verify_payment(self, transaction_reference: str) -> dict:
        ...
        try:
            ...
            raise FlutterwaveVerificationError(f"Verification request failed: {str(e)}")
    ...
    async def initiate_transfer(
        ...
        Returns:
            dict: Transfer response data from Flutterwave

        Raises:
            FlutterwaveError: If transfer initiation fails
        ...
        try:
            ...
            raise FlutterwaveError(f"Transfer request failed: {e.response.text}")
        except httpx.HTTPError as e:
            logger.error(f"Transfer HTTP error: {e}")
            raise FlutterwaveError(f"Transfer request failed: {e}")
        except httpx.RequestError as e:
            # Network error
            logger.error(f"Transfer network error: {e}")
            raise FlutterwaveError(f"Network error during transfer: {e}")

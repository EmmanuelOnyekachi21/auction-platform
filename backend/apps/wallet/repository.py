...
class WalletRepository:
    ...
    async def get_by_id_with_lock(self, wallet_id: UUID) -> Wallet | None:
        """Get wallet by ID with pessimistic lock (SELECT FOR UPDATE).

        REASONING: Used in webhook handler to prevent race conditions when
        multiple webhooks arrive simultaneously for the same wallet.
